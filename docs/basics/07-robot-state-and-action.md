...
接触也不是单帧阈值就能完全描述。碰撞脉冲、稳定夹持和缓慢滑移具有不同时间模式，因此本体与触觉通常需要保留历史。第 12 章会进一步讨论时间建模。
...
这称为 action chunk。它能减少逐步推理开销并表达连贯运动，却会增加开环时间；执行时通常只采用前几步，然后根据新观测重新规划。第 16 章会专门讨论这一问题。
